...
function initMasterSheet(urlOrId) {
  ...
  if (!countries) {
    ...
    // Upgrade existing tab: add valid_leading_digits column if missing
    const headers = countries.getRange(1, 1, 1, countries.getLastColumn()).getValues()[0];
    if (!headers.includes('valid_leading_digits')) {
      const newCol = countries.getLastColumn() + 1;
      countries.getRange(1, newCol).setValue('valid_leading_digits').setFontWeight('bold');

      // Fill in SG = 8,9; leave all others blank
      const data = countries.getRange(2, 1, countries.getLastRow() - 1, 1).getValues();
      for (let i = 0; i < data.length; i++) {
        if (String(data[i][0]).trim().toUpperCase() === 'SG') {
          countries.getRange(i + 2, newCol).setValue('8,9');
          break;
        }
      }
    }
  ...
  // Always ensure column notes are present (idempotent)
  const leadingDigitsCol = countries.getRange(1, 1, 1, countries.getLastColumn())
    .getValues()[0].indexOf('valid_leading_digits') + 1;
  if (leadingDigitsCol > 0) {
    ...
    countries.getRange(1, leadingDigitsCol).setNote(
